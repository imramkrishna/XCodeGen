...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 border-b backdrop-blur-md transition-all duration-200 ${
      isScrolled 
        ? 'border-slate-700/40 bg-slate-900/95 shadow-lg shadow-slate-900/20' 
        : 'border-transparent bg-slate-900'
    }`}>
      <div className="container px-4 mx-auto">
        <div className="flex items-center justify-between py-4">
          {/* Logo - Using the SVG directly */}
          <Link 
            to="/" 
            className="flex items-center gap-2.5 group"
            aria-label="HexaDev Home"
          >
            <svg 
          ...
          {/* Mobile menu toggle */}
          <button 
            className="flex items-center justify-center p-2 text-gray-400 rounded-md lg:hidden hover:text-white focus:outline-none"
      ...
      <div 
        className={`lg:hidden overflow-hidden transition-all duration-300 ${
          mobileMenuOpen ? 'max-h-96 border-t border-slate-800/50' : 'max-h-0'
        }`}
      >
...
const NavItem: React.FC<NavItemProps> = ({ to, label, hasDropdown = false }) => {
  ...
  return (
    <li className="relative mx-1 group">
      ...
      >
        {label}
        {hasDropdown && <ChevronDown className="w-4 h-4 ml-1 transition-transform group-hover:rotate-180" />}
      </Link>
      
      {hasDropdown && (
        <div className="absolute left-0 z-10 w-48 p-2 mt-1 transition-all duration-150 origin-top-left transform scale-95 border rounded-md shadow-lg opacity-0 pointer-events-none bg-slate-800 border-slate-700/50 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100">
          <Link to="#web-apps" className="block px-4 py-2 text-sm rounded text-slate-300 hover:bg-slate-700/50 hover:text-white">
            Web Applications
          </Link>
          <Link to="#mobile-apps" className="block px-4 py-2 text-sm rounded text-slate-300 hover:bg-slate-700/50 hover:text-white">
            Mobile Apps
          </Link>
          <Link to="#landing-pages" className="block px-4 py-2 text-sm rounded text-slate-300 hover:bg-slate-700/50 hover:text-white">
            Landing Pages
          </Link>
        </div>
      )}
...
// Mobile Nav Item Component - Fixed with proper TypeScript types and hash navigation handling
interface MobileNavItemProps {
  to: string;
  label: string;
}

const MobileNavItem: React.FC<MobileNavItemProps> = ({ to, label }) => {
  const location = useLocation();
  ...
  // Handle hash navigation for mobile menu
  const handleClick = () => {
    if (to.startsWith('#')) {
